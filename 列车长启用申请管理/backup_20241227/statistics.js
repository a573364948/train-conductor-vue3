...
function openEditQuotaModal() {
  ...
  departments.forEach(dept => {
    ...
    if (input) {
      input.value = departmentQuotas[dept] || 0;
      
      // 更新当前人数显示
      const currentCount = document.querySelector(`.current-count[data-department="${dept}"]`);
      if (currentCount) {
        currentCount.textContent = getConductorsByDepartmentAndStatus(dept, '正式启用').length + 
          getConductorsByDepartmentAndStatus(dept, '临时启用').length;
      }
    }
  });
